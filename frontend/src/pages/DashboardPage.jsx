...
import {
  BookOpen, Brain, ChevronRight, ChevronLeft,
  RotateCcw, Beaker, Check, Info, Monitor, Activity
} from 'lucide-react';
  ...
  return (
    <div className="range-wrap">
      <div className="range-header">
        ...
        <motion.span
          className={`range-value-badge ${level}`}
          key={`${value}-${level}`}
      ...
      <div style={{ position: 'relative', padding: '0.5rem 0' }}>
        <motion.div
          style={{
            ...
            height: 6, borderRadius: 3, pointerEvents: 'none', zIndex: 1, background: trackColor,
          }}
...
function AnimatedGauge({ value, color }) {
  ...
  const trackD = `M 8 65 A 52 52 0 0 1 112 65`;
...
// ── Step form content direction ──────────────────────────────────────────────
const STEP_DIRS = { forward: 1, backward: -1 };
...
export default function DashboardPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const resp = await axios.post(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/analysis/predict`,
        form,
  ...
  const slideVariants = {
    initial: { opacity: 0, x: stepDir * 45 },
    animate: { opacity: 1, x: 0, transition: { duration: 0.38, ease: [0.22, 1, 0.36, 1] } },
    exit:    { opacity: 0, x: stepDir * -45, transition: { duration: 0.22, ease: 'easeIn' } },
  };
  ...
  return (
    <AnimatedPageWrapper>
      ...
      <div style={{ marginBottom: '1.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        >
          <motion.button className="btn btn-secondary btn-sm" onClick={handleReset} whileHover={{ y: -1 }} whileTap={{ scale: 0.96 }}>
            <RotateCcw size={14} /> Reset
          </motion.button>
          <motion.button className="btn btn-ghost btn-sm" onClick={handleLoadGood} whileHover={{ y: -1 }} whileTap={{ scale: 0.96 }}>
            <Beaker size={14} /> Sample: Good
          </motion.button>
          <motion.button className="btn btn-ghost btn-sm" onClick={handleLoadRisk} whileHover={{ y: -1 }} whileTap={{ scale: 0.96 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 310px', gap: '1.5rem', alignItems: 'start' }}>
          ...
          >
            {STEPS.map((s, i) => (
              <React.Fragment key={i}>
                <div className="step-item" onClick={() => goStep(i)} style={{ cursor: 'pointer' }}>
                  <motion.div
                  ...
                  <span className={`step-label ${step === i ? 'active' : step > i ? 'done' : ''}`}>
                    {s.label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
              ...
              <motion.div key="step0" variants={slideVariants} initial="initial" animate="animate" exit="exit">
                <div className="form-section">
                  ...
                  <div className="input-grid">
                    ...
                    <SliderField id="late_submission_count" label="Late Submission Count" hint="Number of late assignments this semester (0 = best)" min={0} max={20} value={form.late_submission_count} onChange={handleChange} />
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem' }}>
                  <GradientButton onClick={() => goStep(1)}>
            ...
            {step === 1 && (
              <motion.div key="step1" variants={slideVariants} initial="initial" animate="animate" exit="exit">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1rem' }}>
                  <GradientButton variant="secondary" onClick={() => goStep(0)}>
                    <ChevronLeft size={16} /> Back
                  </GradientButton>
                  <GradientButton onClick={() => goStep(2)}>
            ...
            {step === 2 && (
              <motion.div key="step2" variants={slideVariants} initial="initial" animate="animate" exit="exit">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem' }}>
                  <GradientButton variant="secondary" onClick={() => goStep(1)}>
                    <ChevronLeft size={16} /> Back
                  </GradientButton>
                  <GradientButton size="lg" loading={isSubmitting} disabled={isSubmitting} onClick={handleSubmit}>
        ...
        >
          ...
          <div style={{ textAlign: 'center', marginBottom: '1.25rem' }}>
            <AnimatedGauge value={readiness} color={riskColor} />
            ...
            <motion.span
              className={`badge ${readiness >= 70 ? 'badge-success' : readiness >= 45 ? 'badge-warning' : 'badge-danger'}`}
          ...
          <motion.div variants={staggerContainer} initial="initial" animate="animate">
            {[
              ...
              <motion.div key={i} className="preview-metric" variants={staggerItem}>
